...
function Monitoring({ showToast }) {
  ...
  const fetchInstances = async () => {
    setLoading(true)
    try {
      let url = '/workflow/monitoring/instances'
      const params = []
      ...
      if (isSuccess) {
        ...
        if (result.data && result.data.length > 0 && !selectedInstance) {
          loadInstanceDetails(result.data[0].instance_id, result.data)
        }
      } else {
        showToast(result?.Error?.Error_message || result?.message || 'Failed to fetch instances', 'error')
      }
    } catch (e) {
      showToast('Network error while fetching instances', 'error')
    } finally {
      setLoading(false)
    }
  }
  ...
  const loadInstanceDetails = async (instanceId, instancesList = instances) => {
    ...
    try {
      // 1. Fetch Variables
      const varRes = await fetch(`/workflow/monitoring/instances/${instanceId}/variables`)
      const varData = await varRes.json()
      const isVarSuccess = varData && (varData.status === 'success' || (varData.Error && !varData.Error.Error));
      if (isVarSuccess) setVariables(varData.data || {})

      // 2. Fetch Activity logs
      const logRes = await fetch(`/workflow/monitoring/instances/${instanceId}/logs`)
      const logData = await logRes.json()
      const isLogSuccess = logData && (logData.status === 'success' || (logData.Error && !logData.Error.Error));
      if (isLogSuccess) setLogs(logData.data || [])

      // 3. Fetch transition history
      const histRes = await fetch(`/workflow/monitoring/instances/${instanceId}/history`)
      const histData = await histRes.json()
      const isHistSuccess = histData && (histData.status === 'success' || (histData.Error && !histData.Error.Error));
      if (isHistSuccess) setHistory(histData.data || [])
