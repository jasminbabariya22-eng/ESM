...
# Central constants - import here so every helper in this module can use them
from app.core.constants import (
    ApprovalStatus,
    RoleCode,
    RoleName,
    approval_status_name as _approval_status_name_helper,
)
...
# Get Status ID
def get_status_id(db: Session, status_name: str, default_val: Optional[int] = None):
...
def create_update_risk(db: Session, data, current_user):
    ...
    try:
        ...
        logger.info("========== CREATE / UPDATE RISK ==========")
        ...
        if to_int(register_data.risk_register_id) == 0:
            ...
            db.flush()
            if (
                risk.risk_status == opened_status
                and user_type_name.upper() in ["RISK OWNER", "RISK_OWNER", "ADMIN"]
            ):
                

                logger.info(f"Starting workflow for Risk ID: {risk.risk_register_id}")
                ...
                logger.info(f"Workflow created successfully for Risk ID: {risk.risk_register_id}")

                # Queue email only after workflow succeeds
            ...
            if (
                ...
                logger.info(f"Resubmitting workflow for Risk ID: {risk.risk_register_id}")
                ...
                workflow_service.submit(
                    ...
                    entity_id=risk.risk_register_id,
                    user_id=current_user["id"],
                    remarks="Risk updated and resubmitted"
                )

                logger.info(f"Workflow resubmitted successfully for Risk ID: {risk.risk_register_id}")
...
def get_risk_by_user(db, user_id):
    ...
    user_type_name = user_record.user_type.name if user_record and user_record.user_type else ""

    if user_type_name.upper() == "ADMIN":
        risks = db.query(RiskRegister).filter(RiskRegister.is_deleted == 0).all()
    else:
        # Get active visibility instance IDs
        visible_recs = db.query(WorkflowVisibility).filter(
            WorkflowVisibility.user_id == user_id,
            WorkflowVisibility.visibility == 1
        ).all()
        visible_instance_ids = [r.instance_id for r in visible_recs]

        # Query corresponding risk register IDs
        ...
        try:
            instances = wf_db.query(SpiffWorkflowInstance).filter(
                SpiffWorkflowInstance.instance_id.in_(visible_instance_ids),
                SpiffWorkflowInstance.entity_type == "Risk"
            ).all()
            visible_risk_ids = [inst.entity_id for inst in instances]
        finally:
            if db.bind.dialect.name != "sqlite":
                wf_db.close()
        ...
        risks = db.query(RiskRegister).filter(
            ...
            or_(
                ...
                RiskRegister.risk_co_owner_id == user_id,
                RiskRegister.risk_register_id.in_(visible_risk_ids)
            )
        ).all()
...
def get_approval_status_name(status):
    """Delegates to the central constants helper (backward-compatible)."""
    return _approval_status_name_helper(status)
...
def get_risk_by_dept(db, dept_id,financial_year,current_user):  
    ...
    try:
        ...
        no_rejection_condition = and_(
            ...
            or_(
                RiskRegister.risk_function_head_approval_status.is_(None),
                RiskRegister.risk_function_head_approval_status != int(ApprovalStatus.REJECTED)
            ),

            or_(
                RiskRegister.risk_manager_approval_status.is_(None),
                RiskRegister.risk_manager_approval_status != int(ApprovalStatus.REJECTED)
            ),

            or_(
                RiskRegister.risk_head_approval_status.is_(None),
                RiskRegister.risk_head_approval_status != int(ApprovalStatus.REJECTED)
            )
        )
        ...
        # Resolve visible risk IDs from workflow_visibility table
        from app.models.workflow_visibility import WorkflowVisibility
        from app.workflow.persistence.models import SpiffWorkflowInstance

        visible_instance_records = db.query(WorkflowVisibility).filter(
            WorkflowVisibility.user_id == current_user["id"],
            WorkflowVisibility.visibility == 1
        ).all()
        visible_instance_ids = [r.instance_id for r in visible_instance_records]
        ...
        is_admin = user_type_name.upper() == "ADMIN"
