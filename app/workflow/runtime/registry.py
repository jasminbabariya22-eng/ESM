...
class CreateRiskActivity(BaseActivity):
    def __init__(self, risk_db_service: Any = None):
        # Dynamic Dependency Injection (Repository/Service layer)
        self.risk_db_service = risk_db_service

    def validate(self, context: WorkflowContext) -> bool:
        # Validate that the necessary parameters exist
        return context.get_variable("risk_name") is not None

    def execute(self, context: WorkflowContext) -> Dict[str, Any]:
        risk_name = context.get_variable("risk_name")
        print(f"[Activity] Creating risk '{risk_name}' via injected DB service: {self.risk_db_service}")
        
        # In a real setup, we would call: self.risk_db_service.create(...)
        context.set_variable("risk_status", "Draft")
        return {"risk_id": 101, "status": "Draft"}

    def rollback(self, context: WorkflowContext) -> Dict[str, Any]:
        # Compensation: remove the created draft risk if something fails
        print("[Activity] Rollback CreateRisk: removing draft risk from database.")
...
class ApproveRiskActivity(BaseActivity):
    def validate(self, context: WorkflowContext) -> bool:
        return context.get_variable("approval_level") is not None

    def execute(self, context: WorkflowContext) -> Dict[str, Any]:
        level = context.get_variable("approval_level")
        print(f"[Activity] Approving risk at level {level}")
        return {"approved": True, "level": level}

    def rollback(self, context: WorkflowContext) -> Dict[str, Any]:
        # Compensation: revert approval
        print("[Activity] Rollback ApproveRisk: reverting approval level status.")
        return {"rollback_complete": True}


@registry.register("RejectRisk")
class RejectRiskActivity(BaseActivity):
    def validate(self, context: WorkflowContext) -> bool:
        return context.get_variable("remark") is not None

    def execute(self, context: WorkflowContext) -> Dict[str, Any]:
        remark = context.get_variable("remark")
        print(f"[Activity] Rejecting risk with remark: {remark}")
        return {"approved": False, "remark": remark}

    def rollback(self, context: WorkflowContext) -> Dict[str, Any]:
        print("[Activity] Rollback RejectRisk: reverting rejection remarks.")
        return {"rollback_complete": True}
