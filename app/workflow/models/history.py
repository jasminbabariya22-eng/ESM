...
class WorkflowHistory(WorkflowBase):
    ...
    instance_id: Mapped[int] = mapped_column(
        Integer,
        ForeignKey(f"{settings.WORKFLOW_DB_SCHEMA}.workflow_instance.instance_id"),
        nullable=False
    )
    from_state_id: Mapped[Optional[int]] = mapped_column(
        Integer,
        ForeignKey(f"{settings.WORKFLOW_DB_SCHEMA}.workflow_state.state_id"),
        nullable=True
    )
    to_state_id: Mapped[Optional[int]] = mapped_column(
        Integer,
        ForeignKey(f"{settings.WORKFLOW_DB_SCHEMA}.workflow_state.state_id"),
        nullable=True
    )
    ...
    performed_role: Mapped[Optional[str]] = mapped_column(String(20), nullable=True)
    remarks: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    performed_on: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)
    ...
    from_state: Mapped[Optional["WorkflowState"]] = relationship(
        "WorkflowState",
        foreign_keys=[from_state_id]
    )
    to_state: Mapped[Optional["WorkflowState"]] = relationship(
        "WorkflowState",
